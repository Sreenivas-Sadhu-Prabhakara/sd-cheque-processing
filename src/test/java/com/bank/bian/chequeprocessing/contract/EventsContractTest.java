package com.bank.bian.chequeprocessing.contract;
...
class EventsContractTest {
    ...
    void codeTopicsAreDeclaredInTheEventContract() throws Exception {
        ...
        assertThat(events.get("service")).isEqualTo("sd-cheque-processing");
        Set<String> topics = ((List<Map<String, Object>>) events.get("publishes")).stream()
                .map(t -> String.valueOf(t.get("topic")))
                .collect(Collectors.toSet());
        assertThat(topics).as("ChequeProcessingService.TOPIC_CHEQUES must be a declared publish topic").contains(ChequeProcessingService.TOPIC_CHEQUES);
